...
import { Clock, CheckCircle, XCircle, AlertTriangle, ArrowLeft, User } from 'lucide-react';
...
export default function GeneralClockPage() {
  ...
  const loadEmployee = async (employeeId: string) => {
    try {
      ...
      const shiftData = await getActiveShift(employeeId);
  ...
  // Si pas d'employé sélectionné, afficher la sélection
  if (!employee) {
    return (
        ...
        <div className="max-w-md mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            <h1 className="text-2xl font-bold text-gray-800 mb-4 flex items-center space-x-2">
              ...
              <span>Sélection Employé</span>
            </h1>
            <p className="text-gray-600 mb-6">Choisissez votre nom dans la liste :</p>
            
            {/* Liste des employés - à remplacer par une vraie liste */}
            <div className="space-y-3">
              <button
                onClick={() => handleEmployeeSelect('emp_1')}
                className="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-4 px-6 rounded-lg transition-colors duration-200 text-left"
              >
                Marie Dubois - Manager
              </button>
              <button
                onClick={() => handleEmployeeSelect('emp_2')}
                className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-4 px-6 rounded-lg transition-colors duration-200 text-left"
              >
                Jean Martin - Cuisinier
              </button>
              <button
                onClick={() => handleEmployeeSelect('emp_3')}
                className="w-full bg-purple-500 hover:bg-purple-600 text-white font-bold py-4 px-6 rounded-lg transition-colors duration-200 text-left"
              >
                Sophie Bernard - Serveuse
              </button>
            </div>
          </div>
        </div>
